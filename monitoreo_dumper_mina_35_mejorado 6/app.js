...
async function refreshCameraList(){
 ...
 try{
  ...
  if([...cameraSelect.options].some(o=>o.value===current)) cameraSelect.value=current;
 }catch(e){}
}
...
async function startCamera(){
 ...
 const facingMode=cameraMode?.value||'environment';
...
function useExternalStream(mediaStream){
 if(stream) stopCamera();
 stream=mediaStream;
...
if(cameraSelect) cameraSelect.addEventListener('change',()=>{if(stream) startCamera()});
if(cameraMode) cameraMode.addEventListener('change',()=>{if(stream&&!cameraSelect.value) startCamera()});
...
function publish(live){
 const cmp=live.detected?comparisonForLog(live):null;
 const enriched={...live,acceptance:cmp?cmp.acceptance:0,estadoComparacion:cmp?cmp.state:'Sin deteccion',updated:new Date().toISOString()};
 localStorage.setItem('dumperLive',JSON.stringify(enriched));
 const th=JSON.parse(localStorage.getItem('dumperTheory')||'{}');
 if(window.DumperFirebase?.isReady?.()){
  window.DumperFirebase.publishFromLocal(enriched,th,{history:live.detected&&(live.speedKmh||0)>0.02}).catch(()=>{});
 }
}
...
refreshCameraList();
window.DumperCamera={startCamera,stopCamera,useExternalStream,refreshCameraList};
