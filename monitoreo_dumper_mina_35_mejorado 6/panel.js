...
function firebaseToLocal(data){
 if(!data) return null;
 const live={
  detected:data.deteccion!==false,
  speedKmh:Number(data.velocidadReal||0),
  distanceM:Number(data.distanciaReal||0),
  cycles:Number(data.ciclos||0),
  confidence:Number(data.confianzaVisual||0),
  acceptance:Number(data.aceptacion||0),
  offRoute:(data.estado==='CRITICO'||data.estado==='CRÍTICO')&&Math.abs(Number(data.diferenciaVelocidad||0))>0,
  stoppedTooLong:false,
  updated:data.fechaHora||''
 };
 const th={
  avgSpeed:Number(data.velocidadTeorica||0),
  dist:Number(data.distanciaTeorica||0),
  cycle:Number(data.tiempoCicloTeorico||0),
  net:Number(data.productividadTeorica||0),
  effLoad:Number(data.cargaTeorica||0)
 };
 return {live,th};
}

function sourceData(){
 const remote=firebaseToLocal(firebaseCurrent);
 if(remote) return remote;
 return {
  live:JSON.parse(localStorage.getItem('dumperLive')||'{}'),
  th:JSON.parse(localStorage.getItem('dumperTheory')||'{}')
 };
}
...
function renderFirebaseCards(data){
 if(!data) return;
 setText('velocidadReal',fmt(Number(data.velocidadReal||0))+' km/h');
 setText('velocidadTeorica',fmt(Number(data.velocidadTeorica||0))+' km/h');
 setText('cargaReal',fmt(Number(data.cargaReal||0))+' t');
 setText('cargaTeorica',fmt(Number(data.cargaTeorica||0))+' t');
 setText('productividadReal',fmt(Number(data.productividadReal||0))+' t/h');
 setText('productividadTeorica',fmt(Number(data.productividadTeorica||0))+' t/h');
 setText('diferenciaProductividad',fmt(Number(data.diferenciaProductividad||0))+' t/h');
 setText('estadoDumper',data.estado||'-');
 setText('fechaHora',data.fechaHora||'-');
 setText('dispositivoOrigen',data.dispositivoOrigen||'-');
}

function renderFirebaseHistory(rows=firebaseHistory){
 const body=g('tablaHistorial');
 if(!body) return;
 if(!rows.length){
  body.innerHTML='<tr><td colspan="9">Sin datos Firebase. Configura <code>config.js</code> y prueba una actualización.</td></tr>';
  return;
 }
 body.innerHTML=rows.slice(-20).reverse().map(reg=>`
  <tr>
   <td>${reg.fechaHora||'-'}</td>
   <td>${fmt(Number(reg.velocidadReal||0))}</td>
   <td>${fmt(Number(reg.velocidadTeorica||0))}</td>
   <td>${fmt(Number(reg.cargaReal||0))}</td>
   <td>${fmt(Number(reg.cargaTeorica||0))}</td>
   <td>${fmt(Number(reg.productividadReal||0))}</td>
   <td>${fmt(Number(reg.productividadTeorica||0))}</td>
   <td>${fmt(Number(reg.diferenciaProductividad||0))}</td>
   <td>${reg.estado||'-'}</td>
  </tr>
 `).join('');
}

function initFirebasePanel(){
 const applyStatus=(detail)=>{
  const box=g('firebaseStatus');
  if(!box) return;
  box.textContent=detail.message;
  box.className=detail.state==='connected'?'alert ok':(detail.state==='error'?'alert bad':'alert');
 };
 const attach=()=>{
  if(!window.DumperFirebase) return;
  window.DumperFirebase.onStatus(applyStatus);
  if(window.DumperFirebase.isReady()){
   window.DumperFirebase.listenCurrent((data)=>{
    firebaseCurrent=data;
    renderFirebaseCards(data);
    update();
   });
   window.DumperFirebase.listenHistory((rows)=>{
    firebaseHistory=rows;
    renderFirebaseHistory(rows);
   },30);
  }
 };
 window.addEventListener('dumper-firebase-ready',attach);
 window.addEventListener('dumper-firebase-status',e=>applyStatus(e.detail));
 attach();
 const btn=g('testFirebaseBtn');
 if(btn){
  btn.addEventListener('click',async()=>{
   if(!window.DumperFirebase?.isReady?.()){
    g('firebaseStatus').textContent='Firebase no está conectado. Pega firebaseConfig real en config.js.';
    g('firebaseStatus').className='alert bad';
    return;
   }
   await window.DumperFirebase.testUpdate();
  });
 }
}
