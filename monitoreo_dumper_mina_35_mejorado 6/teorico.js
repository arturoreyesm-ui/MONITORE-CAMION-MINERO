...
function calc(){
  ...
  const theory={dist,vLoaded,vEmpty,cap,fill,load,dump,delay,disp,util,outMin,retMin,cycle,cyclesHour,effLoad,gross,net,avgSpeed,updated:new Date().toISOString()};
  localStorage.setItem('dumperTheory',JSON.stringify(theory));
  const live=JSON.parse(localStorage.getItem('dumperLive')||'{}');
  if(window.DumperFirebase?.isReady?.()){
    window.DumperFirebase.publishFromLocal(live,theory,{history:false,force:true}).catch(()=>{});
  }
}
